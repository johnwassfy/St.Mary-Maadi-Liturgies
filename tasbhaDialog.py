...
class TasbhaSelectionDialog(QDialog):
    ...
    def create_photo_panel(self):
        ...
        try:
            photo_label = QLabel()
            pixmap = QPixmap(relative_path(r"Data\الصور\داود.jpg"))
            
            # Make photo larger while maintaining aspect ratio
            pixmap = pixmap.scaled(180, 280, Qt.KeepAspectRatio, Qt.SmoothTransformation)
            ...
            fallback = QLabel("صورة داود النبي")
    ...
    def create_buttons_panel(self):
        ...
        midnight_button = HoverButton("تسبحة نصف الليل")
        midnight_button.setMinimumHeight(90)
        midnight_button.setCursor(Qt.PointingHandCursor)
        midnight_button.setStyleSheet("""
            QPushButton {
                background-color: rgba(240, 240, 240, 200);
                border: 1px solid #c4c4c4;
                border-radius: 15px;
        ...
        # Style and set icon
        self.style_button(midnight_button, "fa5s.moon")
        
        # Add custom CSS for center alignment with icon
        midnight_button.setStyleSheet(midnight_button.styleSheet() + """
            QPushButton {
                text-align: center;
            }
        """)
        
        midnight_button.clicked.connect(lambda: self.button_clicked("midnight"))
        
        # Evening button with centered content
        evening_button = HoverButton("تسبحة عشية")
        evening_button.setMinimumHeight(90)
        evening_button.setCursor(Qt.PointingHandCursor)
        evening_button.setStyleSheet("""
            QPushButton {
                background-color: rgba(240, 240, 240, 200);
                border: 1px solid #c4c4c4;
                border-radius: 15px;
        ...
        # Style and set icon
        self.style_button(evening_button, "fa5s.cloud-sun")
        
        # Add custom CSS for center alignment with icon
        evening_button.setStyleSheet(evening_button.styleSheet() + """
            QPushButton {
                text-align: center;
            }
        """)
        
        evening_button.clicked.connect(lambda: self.button_clicked("evening"))
        
        buttons_layout.addWidget(midnight_button)
